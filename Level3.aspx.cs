...
namespace GraWPanstwa.Poziomy
{
    public partial class Level3 : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            gracz.Nazwa = (string)Session["graczLevel2Login"];
            gracz.Punkty = (int)Session["graczLevel2Punkty"];
            string[] s = new string[] { "U", "Z", "B", "E", "K", "I", "S", "T", "A", "N" };
            string[] poZmieszaniu;
            if (!IsPostBack)
            {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            Gracz gracz = new Gracz
            {
                Nazwa = (string)Session["graczLevel2Login"],
                Punkty = (int)Session["graczLevel2Punkty"]
            };
            Session["graczLevel3Login"] = gracz.Nazwa;
            BazaPanstw baza = new BazaPanstw();
            if (baza.Panstwa().Contains(txtAnswear.Text) && counter == 0)
            {
                gracz.Punkty += 5;
                Session["graczLevel3Punkty"] = gracz.Punkty;
                Response.Redirect("/Poziomy/Level4.aspx");
            }
